...
export function astar(grid, startNode, finishNode, diagonal) {
  ...
  while (open_list.length != 0) {
    ...
    if (current === null) {
      console.log("Error: A* = Unable to retrieve node with the least f value");
    ...
    if (index > -1) open_list.splice(index, 1);
    ...
    for (const neighbor of current.side_neighbor) {
      ...
      if (g_temp < neighbor.g) {
        ...
        if (!isNeighborInOpenSet(neighbor, open_list)) open_list.push(neighbor);
      }
    }
